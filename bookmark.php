...
get_header();
$bk_post_count = cs_get_option('post_count_bk');
$bk_post_order = cs_get_option('bk-order');
$no_bx_save_title = cs_get_option('no_bx_save_title');
$no_bx_desc = cs_get_option('no_bx_desc');
?>
            ...
            if (is_user_logged_in()) {
                ...
                if ($bookmarks) {
                    ...
                    $args = array(
                        'post_type' => 'post',
                        'post__in' => $bookmarks,
                        'posts_per_page' => !empty($bk_post_count) ? $bk_post_count : 9,
                        'order' => $bk_post_order,
                        'paged' => $paged
                    );
                    ...
                    if ($bookmark_query->have_posts()) {
                        ...
                        ?>
                        <div class="empath-pagination bookmark__pafiu">
                            <?php
                    ...
                    <div class="bookmark__no text-center">
                        <i class="fas fa-meh-blank"></i>
                        <?php if(!empty($no_bx_save_title)):?>
                            <h3><?php echo empath_wp_kses($no_bx_save_title);?></h3>
                        <?php endif;?>
                        <p><?php echo empath_wp_kses($no_bx_desc);?></p>
                    </div>
                <?php 
                }
            } else { ?>
                <div class="bookmark__no text-center">
                    ...
                    <h3><?php esc_html_e('Please Login!', 'forcast'); ?></h3>
                    <p><?php esc_html_e('Please log in to view or add your bookmarks.', 'forcast'); ?> </p>
                    <a aria-label="name" class="thm__btn" href="<?php echo esc_url(home_url('/login')); ?>"><?php esc_html_e('Click to Login', 'forcast'); ?></a>
                </div>
            <?php 
            }
            ?>
